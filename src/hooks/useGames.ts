...
const useGames = () => {
    ...
    useEffect(() => {
        ...
        apiClient
            ...
            .then(res => {
                setGames(res.data.results); //* res var is type FetchGamesResponse, so we can then get .results property. res var has property and responses from Axios, use data prop so we can read body of response
                setLoading(false);
            }) 
            .catch(err => {
    ...
    return { games, error, isLoading }; //*return games and error objects to be used in components
}
